...
// __________________________.....AÑADIDAS.....________________________________
...
bool LeerImagen(const char *nombre, Imagen &imagen, TipoImagen &tipo){
  ...
  if (tipo == IMG_PGM){
        vector = LeerImagenPGM(nombre, f, c);
        Imagen imagen_aux(f, c, vector);
        imagen = imagen_aux;
        correcto = true;
  }
  else
...
bool EscribirImagen(const char *nombre, const Imagen &imagen, const TipoImagen &tipo){
  bool correcto = true;
  if (tipo == IMG_PGM){
    correcto = EscribirImagenPGM(nombre, imagen.flat(), imagen.get_filas(), imagen.get_columnas());
  }
  else
    correcto = false;
